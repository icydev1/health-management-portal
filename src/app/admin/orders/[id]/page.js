  ...
  return (
    <div className="min-h-screen bg-gray-50 p-8">
      ...
      <div className="space-y-5">
        ...
        {/* Feedback — only for Completed orders with an assigned freelancer */}
        {order.status === 'Completed' && order.freelancer && (
          <div className="bg-white rounded-xl border-2 border-gray-200 overflow-hidden">
            <div className="flex items-center gap-3 px-5 py-4 border-b border-gray-100 bg-gray-50">
              <div className="w-8 h-8 bg-green-100 rounded-lg flex items-center justify-center text-green-700">
                <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
              ...
              <div>
                <h2 className="font-bold text-gray-900">Freelancer Feedback</h2>
                <p className="text-xs text-gray-400">Rate the performance of {order.freelancer}</p>
              </div>
            </div>
            ...
            <div className="px-5 py-5">
              {feedbackSubmitted ? (
                ...
                <div className="flex flex-col items-center py-6 gap-3">
                  ...
                  <p className="text-lg font-bold text-gray-900">Feedback Submitted</p>
                  <div className="flex items-center gap-1">
                    {[1,2,3,4,5].map((s) => (
                      <svg key={s} className={`w-6 h-6 ${s <= feedback.rating ? 'text-yellow-400' : 'text-gray-200'}`} fill="currentColor" viewBox="0 0 20 20">
                        <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                      </svg>
                    ))}
                  </div>
                  {feedback.comment && (
                    <p className="text-sm text-gray-600 italic max-w-md text-center">"{feedback.comment}"</p>
                  )}
                  <button
                ...
                <div className="space-y-5">
                  ...
                  <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl border border-gray-100">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center text-white font-bold text-sm shrink-0 ${avatarColor(order.freelancer)}`}>
                      {order.freelancer.split(' ').map((n) => n[0]).join('').slice(0, 2).toUpperCase()}
                    </div>
                    <div>
                      <p className="font-bold text-gray-900 text-sm">{order.freelancer}</p>
                      <p className="text-xs text-gray-500">Assigned freelancer · {order.orderNum}</p>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      {[1,2,3,4,5].map((s) => (
                        ...
                        >
                          <svg className={`w-9 h-9 transition-colors ${s <= feedback.rating ? 'text-yellow-400' : 'text-gray-200 hover:text-yellow-300'}`} fill="currentColor" viewBox="0 0 20 20">
                            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                          </svg>
                        </button>
                      ))}
                  ...
                  {/* Comment */}
                  <div>
                    <p className="text-sm font-semibold text-gray-700 mb-2">Comment <span className="text-gray-400 font-normal">(optional)</span></p>
                    <textarea
                      rows={4}
                      value={feedback.comment}
                      onChange={(e) => setFeedback((f) => ({ ...f, comment: e.target.value }))}
                      placeholder={`Share your experience working with ${order.freelancer}…`}
                      className="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-green-600 focus:outline-none text-sm resize-none"
                    />
                  </div>
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-2 px-6 py-2.5 bg-green-700 hover:bg-green-800 disabled:opacity-40 disabled:cursor-not-allowed text-white font-semibold rounded-lg transition-colors text-sm"
                  >
                    <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                      <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                    </svg>
